...
A first roguelite mode. Start it from the forest road: **World I â†’ FOREST RUN / RANDOM ROOMS**.

- A run is four rooms in the same forest clearing. Rooms 1â€“3 are goblin fights, and room 4 is the Goblin Queen.
...
| Part | Rolled |
|---|---|
| Cover | 4â€“8 boulders, fallen logs and stumps (2â€“4 in the Queen's room) |
| Thorns | Patches that hurt while you stand in them: none in room 1 or the boss room, up to one per room number after that |
| Waves | 2â€“3 waves, each one of: **Goblins** (arrive one by one), **Ambush** (the whole group at once), or **Elites** (1â€“2 big, tough, fast goblins). Waves get bigger each room. |

Safety rules keep every room fair. The centre (6.5 m) and both doorways stay clear. Cover keeps its distance from other cover, from thorns and from the tree line. Waves never spawn near the entrance. Each room has an enemy budget of 6 + 5 Ã— the room number.
...
See `Docs/ENVIRONMENT.md` for the pieces: the generated forest kit for trees, logs, stumps, plants, brambles, burrows and the gateway; realistic boulders; Pack Bonus grass; fireflies; the night sky and moonlight. The trail is marked by stepping stones. The exit is a rune gateway where a green wisp appears once the room is clear, and standing stones mark the entrance.

## Testing

- `Tests\run-checks.ps1 -Only ForestRun` plans 1,600 rooms (400 seeds Ã— 4 rooms). It checks each is fair and repeatable and that the rooms vary, then checks that a fight room and the Queen's room are built as planned.

...
- `Enemies/EnemySpawnPoint.cpp`: enemy spawns show no marker by default. The hell rift and its fire are kept for later behind `bShowRift`. The goblin burrow is kept for later too (`UseBurrow`, not used now). Enemies arrive with a puff of smoke, and the old floating labels and debug circles are gone.
